...
function Node({
  ...
  return (
    <g>
      <rect x={x} y={y} width={w} height={54} rx={6} fill="#17191d" stroke={accent} strokeWidth={1} />
      <text x={x + 12} y={y + 22} fill="#fbfbfb" fontSize={11} fontWeight={700} style={mono}>
        {title}
      </text>
      <text x={x + 12} y={y + 40} fill="#69707a" fontSize={9} style={mono}>
        {sub}
      </text>
    </g>
  )
        ...
        <Reveal i={3}>
          <div className="panel noise relative mt-14 overflow-hidden p-4 sm:p-8">
            <svg viewBox="0 0 980 360" className="w-full" role="img" aria-label="SurStor data flow: AI systems and agents write events into the SurStor hash-chained EU storage layer, which serves deployers, post-market monitoring and market surveillance authorities.">
              {/* sources */}
              <text x={10} y={18} fill="#69707a" fontSize={9} letterSpacing={2} style={mono}>
                SOURCES — YOUR STACK
              </text>
              ...
              {/* SurStor core */}
              <text x={335} y={18} fill="#ff9e40" fontSize={9} letterSpacing={2} style={mono}>
                SURSTOR — EU REGIONS ONLY
              </text>
              <rect x={330} y={130} width={300} height={120} rx={10} fill="#121316" stroke="#ff9e40" strokeOpacity={0.5} strokeWidth={1.25} />
              <text x={350} y={158} fill="#fbfbfb" fontSize={13} fontWeight={700} style={mono}>
                hash-chained log pools
              </text>
              <text x={350} y={178} fill="#b8bcc4" fontSize={10} style={mono}>
                append-only · qualified timestamps
              </text>
              <text x={350} y={196} fill="#b8bcc4" fontSize={10} style={mono}>
                DE-FRA · NL-AMS · IE-DUB · SE-STO
              </text>
              <rect x={350} y={212} width={180} height={20} rx={4} fill="rgba(255,158,64,0.1)" />
              <text x={358} y={226} fill="#ff9e40" fontSize={9} style={mono}>
              ...
              {/* consumers */}
              <text x={780} y={18} fill="#69707a" fontSize={9} letterSpacing={2} style={mono}>
                CONSUMERS — PROOF OUT
              </text>
              <Node x={780} y={42} title="deployer console" sub="art. 26(5) oversight" accent="#5fa8c7" />
              <Node x={780} y={152} title="post-market monitor" sub="art. 72 analysis" accent="#5fa8c7" />
